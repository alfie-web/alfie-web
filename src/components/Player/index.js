...
export default function Player({ activeAudio, activeVideo, playerState, changePlayerState, setLoop }) {
	...
	}, [playerState, changePlayerState, stopSound])
	...
	// 		console.log(playerState)	// Так как постоянно всё перерисовывается, работает не корректно, как вариант можно вынести это в redux или сам плеер вынести в отдельную компоненту
	...
	// // }, [playerState])
	// }, [playSound, stopSound, playerState, fakeDuration])
	...
	useEffect(() => {
		audioRef.current.addEventListener('timeupdate', () => {
			...
			// 	// TODO: Останавливаем воспроизведение (или след аудио или repeat)
				...
				if (playerState.isLooped) playSound()
					
				
			}
			// resetSound(currentTime)
		})
	}, [audioRef, fakeDuration, outlineLength, playerState, playSound, stopSound])
	...
	return (
		...
		<div className="App__player">
			<div className="App__player-track">
				<button onClick={ playSound } className="Button Button--active" title={ playerState.isPlaying ? 'Отстановить' : 'Проигрывать' }>
					{ playerState.isPlaying ? <svg viewBox="0 0 36 42" xmlns="http://www.w3.org/2000/svg">
		...
		<div className="App__controls">
				<button className={ classNames('Button', { 'Button--active': playerState.isLooped }) } onClick={ setLoop } title="Зациклить">
