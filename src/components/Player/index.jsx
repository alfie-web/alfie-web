...
// function Player({ setIsPlaying, isPlaying, isLooped, setLoop, activeAudio, activeVideo, goFull, isFullscreen, setIsCanPlay, isCanPlay }) {
...
// 			//  TODO:Очень похожая логика, надо порефакторить
...
// 	// const checkCanPlay = (canPlay) => {
// 	// 	// console.log('Can Play');
// 	// 	setIsCanPlay(canPlay)
...
// 	useEffect(() => {
// 		audioRef.current.addEventListener('canplay', () => setIsCanPlay(true));
// 		return () => audioRef.current.removeEventListener('canplay', () => setIsCanPlay(true));
// 	})

// 	useEffect(() => {
// 		audioRef.current.addEventListener('waiting', () => setIsCanPlay(false));
// 		return () => audioRef.current.removeEventListener('waiting', () => setIsCanPlay(false));
// 	})
...
function Player({ setIsPlaying, isPlaying, isLooped, setLoop, activeAudio, activeVideo, goFull, isFullscreen }) {
	...
	// TODO: Сбрасывать бар когда выбираю время, или делается ресет
	const onProgress = (e) => {
		if (audioRef.current.readyState !== 0) {	// Без этого buffered почемуто выдаёт ошибку
			var loadedTime = e.target.buffered.end(0);
			console.log(loadedTime);
